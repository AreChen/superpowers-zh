...
**个人技能位于你的运行时的技能目录中**（Claude Code 为 `~/.claude/skills/`）——Codex 的路径参见 [codex-tools.md](../using-superpowers/references/codex-tools.md)，Gemini 的路径参见 [gemini-tools.md](../using-superpowers/references/gemini-tools.md)。Codex、Copilot CLI 和 Gemini CLI 也都识别 `~/.agents/skills/` 这一跨运行时别名。
